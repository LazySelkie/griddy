...
Actor.prototype.griddy = function(options={}) {
  ...
  let systemWeightProp = game.settings.get('griddy', `systemWeightProp`)
  ...
  let d = new Dialog({
    ...
    render: async (html)=>{
      ...
      function calcItemWeight(item) { // item.system.props.weight
        let w = foundry.utils.getProperty(item.system, systemWeightProp) * foundry.utils.getProperty(item.system, systemQuanityProp);
        if (item.flags.griddy?.position?.c) {
          w += item.parent.items.filter(i=>i.flags.griddy?.position?.n == item.id).reduce(function (acc, cur) {
            if (cur.flags.griddy?.position?.c) {
              acc += calcItemWeight(cur);
            } else {
              acc += foundry.utils.getProperty(cur.system, systemQuanityProp) * foundry.utils.getProperty(cur.system, systemWeightProp);//cur.system.props.weight;
            }
            return acc;
          }, 0);
        }
      ...
      function roundWeight(weight) {
        // rounds to show 3 numbers after .
      ...
      let itemElements = items.reduce((a,i)=>a+=//${conflicts(i.flags.griddy?.position, i.id, items).length?'conflicts':''}
             ...
             <span class="item-weight" style="font-size:${gridSize/4.5}px;position: absolute; bottom: 2px; left: 2px; color: white; background: rgba(0, 0, 0, 0.5); padding:1px 3px; display: inline">
              ${roundWeight(calcItemWeight(i))} kg
              </span>
             </div>`, ``)
      ...
      html.find('div.item').bind("dragstart", function(e) {
        ...
        this.style.zIndex = 1
        let quantity = Number($(this).find('span.item-quantity').text())
        let split = false
        if ((e.shiftKey||e.ctrlKey) && quantity>1) {
          ...
          if (quantity>1) {
            if (e.ctrlKey) {
              clone.find('span.item-quantity').text(quantity-1)
              $(this).find('span.item-quantity').text(1)
            }
            if (e.shiftKey) {
              clone.find('span.item-quantity').text(Math.ceil(quantity/2))
              $(this).find('span.item-quantity').text(Math.floor(quantity/2))
            }
        ...
        let data = {
          ...
          x: ui.gridDragData.x,
          y: ui.gridDragData.y,
          quantity: Number($(this).find('span.item-quantity').text()),
          split
        }
      ...
      .bind('drop', async function(e){
        ...
        if (i) { // if dropped on another item
          ...
          if (i.name == item.name && i.id != item.id) { // dropped on an item of the same name
            ...
            if (item.id==i.id) return $(e).find('span.item-quantity').text(itemQuantity)
      ...
      html.find('div.item > span.item-quantity').click(function(e){
        e.stopPropagation();
  ...
  Hooks.once("init", ()=>{
    ...
    game.settings.register('griddy', `systemWeightProp`, {
      name: `System Weight Property `,
      hint: `Where in the system data weight is stored.`,
      scope: "world",
      type: String,
      default: "weight",
      config: true,
